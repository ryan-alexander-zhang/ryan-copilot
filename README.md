...
- `alphaxiv-paper-lookup`: 通过 alphaxiv 获取 arXiv 论文的结构化 AI 摘要，避免直接读原始 PDF
  路径: `.codex/skills/alphaxiv-paper-lookup`
- `context-hub-doc-builder`: 从 URL、OpenAPI/Swagger 或本地文件生成
  可 `chub build` 的私有 Context Hub docs，并输出测试 CLI 与手动配置提示
  路径: `.codex/skills/context-hub-doc-builder`
- `github-repo-capability-validator`: 对 GitHub 仓库执行 README-first 的 capability 分析、代码核验、任务拆分和报告输出
  路径: `.codex/skills/github-repo-capability-validator`
- `markdownlint-cli2-validator`: 用 `markdownlint-cli2` 校验和自动修复 Markdown
  路径: `.codex/skills/markdownlint-cli2-validator`
- `obsidian-feishu-group-sender`: 将 Obsidian Markdown 转成 Feishu 机器人消息并生成可执行发送脚本
  路径: `.codex/skills/obsidian-feishu-group-sender`
- `obsidian-telegram-note-sender`: 将 Obsidian Markdown 转成 Telegram Bot API 消息并生成可执行发送脚本
  路径: `.codex/skills/obsidian-telegram-note-sender`
- `product-web-analysis`: 公开产品分析与 `Build MVP` 三报告
  路径: `.codex/skills/product-web-analysis`

### `.copilot/skills`

- `obsidian-feishu-group-sender`: Feishu 群消息发送副本
  路径: `.copilot/skills/obsidian-feishu-group-sender`
- `obsidian-telegram-note-sender`: Telegram 话题消息发送副本
  路径: `.copilot/skills/obsidian-telegram-note-sender`
...
```bash
python3 \
  .codex/skills/context-hub-doc-builder/scripts/scaffold_context_hub_doc.py \
...
```bash
python3 \
  .codex/skills/obsidian-feishu-group-sender/scripts/\
  build_feishu_send_script.py \
...
```bash
python3 \
  .codex/skills/obsidian-telegram-note-sender/scripts/\
  build_telegram_send_script.py \
  --markdown /absolute/path/to/note.md \
