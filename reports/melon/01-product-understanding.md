...
- `支付合作方风险`：如果底层 ACH/支付合作方收紧对高风险行业或成人内容生态的支持，Melon 的业务会受到实质冲击。
...
flowchart LR
  ...
  B --> C[Creator 用 Plaid 连接银行]
  C --> D[Melon 监测上游平台入账]
  D --> E[按分成比例向 Creator 发起扣款]
  E --> F[ACH 清算约 4 个工作日]
  F --> G[周度向 Agency 汇总 payout]
  G --> H[可选: 对 Referral / Chat Team 二次分账]
  B --> I[可选: Invoice Split]
  I --> J[FansMetric 提供 creator 数据]
  J --> K[Melon 通过邮件/SMS 发送支付链接]
  K --> L[Creator 通过银行卡或银行账户付款]
```
...
sequenceDiagram
  ...
  Melon->>Partners: 发起扣款 / ACH 转账
  Partners-->>Melon: 返回清算状态
...
System_Ext(payment, "Dwolla / 其他支付合作方", "ACH 与资金移动")
